...
def decisionTreeScore(x_treino, x_teste, y_treino, y_teste):
...
def optimizeNan(arquivo):
	...
	print('NaN Metascore: ' + str(arquivo['Metascore'].isna().sum()))
	print('NaN Description: ' + str(arquivo['Description'].isna().sum()))
...
def prepare():
	...
	### Remove os NaN
	arquivo = arquivo.replace(np.nan, 0, regex=True)
	### Primeiras amostras
	return arquivo
...
def main():	
	...
	while i < 10:
		...
		result = decisionTreeScore(x_treino, x_teste, y_treino, y_teste)
